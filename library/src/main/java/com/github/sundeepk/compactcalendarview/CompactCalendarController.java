...
class CompactCalendarController {
    ...
    // colors
    private int calenderHeaderBackgroundColor;
    ...
    private int calendarFullLineColor = Color.GRAY;
    private int calendarDashLineColor = Color.GRAY;
    ...
    CompactCalendarController(Paint dayPaint, OverScroller scroller, Rect textSizeRect, AttributeSet attrs,
                              Context context, int calenderHeaderBackgroundColor, int currentDayBackgroundColor, int calenderTextColor,
        ...
        this.calenderHeaderBackgroundColor = calenderHeaderBackgroundColor;
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                calenderHeaderBackgroundColor = typedArray.getColor(R.styleable.CompactCalendarView_compactCalenderHeaderBackgroundColor, calenderHeaderBackgroundColor);
                ...
                calendarFullLineColor =  typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarFullLineColor, calendarFullLineColor);
                calendarDashLineColor =  typedArray.getColor(R.styleable.CompactCalendarView_compactCalendarDashLineColor, calendarDashLineColor);
    ...
    void setCalenderHeaderBackgroundColor(int calenderHeaderBackgroundColor) {
        this.calenderHeaderBackgroundColor = calenderHeaderBackgroundColor;
    }
    ...
    void onMeasure(int width, int height, int paddingRight, int paddingLeft) {
        ...
        heightPerDay = targetHeight > 0 ? targetHeight / 5 : height / 5;
        this.width = width;
        this.distanceThresholdForAutoScroll = (int) (height * 0.50);
        this.height = height;
...
//        return (float) (y0 + ((y1 - y0) * ((x - x0) / (x1 - x0))));
    ...
    void onSingleTapConfirmed(MotionEvent e) {
        ...
        if (Math.abs(accumulatedScrollOffset.y) != Math.abs(height * monthsScrolledSoFar)) {
            return;
        }

        int dayColumn = Math.round((e.getX() - paddingWidth - paddingRight) / widthPerDay);
...
//            handleHorizontalScrolling();
    ...
    private void snapBackScroller() {
        float remainingScrollAfterFingerLifted1 = (accumulatedScrollOffset.y - (monthsScrolledSoFar * height));
        scroller.startScroll(0, (int) accumulatedScrollOffset.y, 0, (int) -remainingScrollAfterFingerLifted1);
    }

    private void handleVerticalScrolling() {
        int velocityY = computeVelocity();
        handleSmoothScrolling(velocityY);
    ...
    private void handleSmoothScrolling(int velocityY) {
        int distanceScrolled = (int) (accumulatedScrollOffset.y - (height * monthsScrolledSoFar));
        ...
        if (velocityY > densityAdjustedSnapVelocity && isEnoughTimeElapsedSinceLastSmoothScroll) {
            scrollPreviousMonth();
        } else if (velocityY < -densityAdjustedSnapVelocity && isEnoughTimeElapsedSinceLastSmoothScroll) {
            scrollNextMonth();
    ...
    private void performScroll() {
        int targetScroll = monthsScrolledSoFar * height;
        float remainingScrollAfterFingerLifted = targetScroll - accumulatedScrollOffset.y;
        scroller.startScroll( 0, (int) accumulatedScrollOffset.y, 0, (int) (remainingScrollAfterFingerLifted),
                (int) (Math.abs((int) (remainingScrollAfterFingerLifted)) / (float) height * ANIMATION_SCREEN_SET_DURATION_MILLIS));
    }
    ...
    private void calculateYPositionOffset() {
    ...
    void drawEvents(Canvas canvas, Calendar currentMonthToDrawCalender, int offset) {
        ...
        if (uniqEvents != null) {
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                float yPosition = weekNumberForMonth * heightPerDay + paddingHeight + accumulatedScrollOffset.y + offset;
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        ...
        //define the dash paint
        Paint dashPaint = new Paint();
        dashPaint.setColor(Color.GRAY);
        dashPaint.setStyle(Paint.Style.STROKE);
        float interval = width/100;
        PathEffect effect = new DashPathEffect(new float[] {interval,interval}, 1);
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            float yPosition = (dayRow-1)* heightPerDay + paddingHeight + accumulatedScrollOffset.y + offset;
            if (xPosition >= growFactor && isAnimatingWithExpose || yPosition >= growFactor) {
                continue;
            }
            int day = ((dayRow - 1) * 7 + dayColumn + 1) - firstDayOfMonth;
            // draw dashline
            drawDashLine(canvas, yPosition, dashPaint);
            //draw circles
            ...
            if (day <= monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH) && day > 0) {
                //change the pait color if the text is in circle
    ...
    private void drawMonthTitle(Canvas canvas){
        int bottomY = Math.round(textSizeRect.height()*1.5f);
        //draw background of header
        dayPaint.setColor(calenderHeaderBackgroundColor);
        dayPaint.setStyle(Paint.Style.FILL);
        canvas.drawRect(new Rect(0, 0, width, bottomY), dayPaint);
        dayPaint.setColor(calenderTextColor);
        canvas.drawLine(0, bottomY, width, bottomY, dayPaint);

        if (shouldDrawDaysHeader) {
            dayPaint.setColor(calenderTextColor);
            dayPaint.setTypeface(Typeface.DEFAULT_BOLD);
            for (int dayColumn = 0; dayColumn < dayColumnNames.length; dayColumn++) {
                float xPosition = widthPerDay * dayColumn + paddingWidth + paddingLeft - paddingRight;
                float yPosition = textSizeRect.height();
                canvas.drawText(dayColumnNames[dayColumn], xPosition, yPosition , dayPaint);
            }
            dayPaint.setTypeface(Typeface.DEFAULT);
        }
    }
    private void drawFullLine(Canvas canvas, float space, int color, Paint paint){
        int original_color = paint.getColor();
        float baseXPosition = paddingWidth + paddingLeft - paddingRight - (float)0.5*widthPerDay;
        paint.setColor(color);
        for (int column = 0; column < dayColumnNames.length; column++) {
            float positionX = widthPerDay * (column + 1) + baseXPosition;
            canvas.drawLine(positionX, 0, positionX, height, paint);
        }
        paint.setColor(original_color);
    }

    private void drawDashLine(Canvas canvas, float positionY, Paint paint){

        float drawInPositionY = positionY - textSizeRect.height()*2;
        canvas.drawLine(0, drawInPositionY, width, drawInPositionY, paint);
    }

    private void drawTopLevel(Canvas canvas){
        // draw fullline only need to draw once, because it doesn't scroll
        drawFullLine(canvas, textWidth * 2, Color.BLACK, dayPaint);
        // draw title block
